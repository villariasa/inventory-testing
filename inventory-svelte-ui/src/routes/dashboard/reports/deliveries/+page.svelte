<script lang="ts">
  ...
  let columns = $derived([
    { key: 'fmt_delivery_date', label: 'Date' },
    { key: 'delivery_reference', label: 'Ref No' },
    ...
    ...(withCostEnabled ? [
      ...
      { key: 'total_cost_display', label: 'Total Cost', align: 'right' as const }
    ] : []),
    { key: 'receiving_unit', label: 'Warehouse' }
  ]);
  ...
  async function handleFilter(filters: any) {
    ...
    const response = await invokeService<any, any[]>('/get-deliveries-report', {
      body: {
        dateFrom: filters.dateFrom,
        dateTo: filters.dateTo,
        units: unitList,
    ...
    if ('data' in response && response.data.success) {
      ...
      rows = list.map((item: any) => {
        const uCost = parseNumber(item.unit_cost);
        const tCost = parseNumber(item.total_cost);

        return {
          ...item,
          cost_display: `PHP ${uCost.toLocaleString('en-US', { minimumFractionDigits: 2 })}`,
          total_cost_display: `PHP ${tCost.toLocaleString('en-US', { minimumFractionDigits: 2 })}`,
          raw_unit_cost: uCost,
          raw_total_cost: tCost
        };
      });
    } else {
      rows = [];
      const errMsg = 'error' in response ? response.error : (response.data?.message || 'Unknown error');
      console.error('[Deliveries Report] Failed to load report data:', errMsg, response);
  ...
  async function exportToPDF() {
    ...
    try {
      ...
      doc.text('Deliveries Report', 14, currentY);
      ...
      const headers = [
        'Date', 'Ref No', 'PO Ref', 'Supplier', 'Category', 'Brand', 'Item', 'Qty'
      ];
      if (withCostEnabled) {
      ...
      autoTable(doc, {
        ...
        columnStyles: {
          7: { halign: 'right' }, // Qty
          ...(withCostEnabled ? {
            8: { halign: 'right', cellWidth: 'wrap' }, // Unit Cost
            9: { halign: 'right', cellWidth: 'wrap' }  // Total Cost
          } : {})
        },

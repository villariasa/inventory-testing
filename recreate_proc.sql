...
DROP PROCEDURE IF EXISTS getInventoryItem;
...
BEGIN
    ...
    --     "barcode": "INV-000001"  // <-- Now supports barcode searching!
    -- }'

    -- Increase group_concat_max_len to prevent JSON truncation
    ...
    IF bolProceed THEN
        ...
        -- FIX 1: Convert 0 to NULL so our WHERE logic can easily detect a blank category search
        ...
        -- Extract barcode for scanner search functionality
        ...
        -- FIX 2: Rebuilt the dynamic WHERE clause to elegantly handle the No-Image filter
        IF bolGetOne THEN
            ...
            END IF;
        ELSE
            -- Start with a generic truth so we can cleanly append 'AND' statements
            ...
            END IF;
            -- Category/Image filter logic
            IF intItemCategoryId IS NOT NULL THEN
                SET whereCredentials = CONCAT(whereCredentials, ' AND ii.item_category_id = ', intItemCategoryId);
            ELSE
                -- If category is 0 (NULL) AND there is NO active query (barcode or description), filter to populate ONLY items that do NOT have an image
